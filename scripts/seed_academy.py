...
Temporal — durable workflows. Publishing a course for the AI assistant is a four-step pipeline: extract text → chunk it → embed chunks → index them in Qdrant, then mark READY. Naive version: run all four in an endpoint (blocks — you know better now) or a Celery task (better, but if the worker dies at step 3, restarting redoes 1 and 2, or corrupts state). Temporal's pitch: write the pipeline as ordinary code, and the engine checkpoints every step — crash at step 3 means resume AT step 3, on any worker, tomorrow if needed. The rule of thumb encoded in this repo: one-shot side effect → Celery; multi-step process that must survive crashes → Temporal. The enrollment workflow will even derive its workflow ID from (student_id, course_id), making duplicate submissions structurally idempotent — the same idea you saw as a unique constraint, lifted into the workflow layer.
...
The AI assistant — RAG over course content. You built the mental model in Foundations' sibling course; here's the wiring: course text → chunks → embeddings → Qdrant (filtered by course_id), and at question time: embed the question → retrieve nearest chunks → prompt the LLM to answer ONLY from them, streaming tokens to the browser. The provider is already abstracted (app/ai/llm.py — swap Anthropic/OpenAI/Groq by env var); retrieval and the agent graph are the scaffolded stubs.
